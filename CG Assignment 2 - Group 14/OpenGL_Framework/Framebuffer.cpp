...
vec3i Framebuffer::_MaxFramebufferSize; // Width, Height, and Layers (Depth) texture dimensions
...
{
	addDepthTarget();
	addColorTarget(GL_RGB8); // al
	addColorTarget(GL_RGB8); // nor
	addColorTarget(GL_R11F_G11F_B10F); // emi
	addColorTarget(GL_R8); //rou
	addColorTarget(GL_R8); // mat
...
void GBuffer::setDebugNames()
{
	...
	setOpenGLName(GL_TEXTURE, _Color._Tex[4].getID(), "GBuffer Metallic");
}
